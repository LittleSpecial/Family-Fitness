...
【类型A - 单次运动记录】
特征：显示某一次具体运动的详细数据（如户外步行、跑步、网球等）
包含：运动类型、运动时长、卡路里、可能有步数、心率等

【类型B - 每日步数总结】
特征：显示全天步数统计，通常标题为"步数"，显示一天的总步数、总消耗、总距离
包含：总步数（通常是较大的数字如15,796步）、总消耗卡路里、总距离、时间范围（如00:00-23:59）
不包含：单次运动时长（因为是全天累计）
...
5. 如果看到"网球"就返回"网球"，看到"跑步"就返回"跑步"，看到"骑行"就返回"骑行"，看到"户外步行"就返回"户外步行"
6. 绝对不能猜测或推断，必须是图中明确显示的文字
7. 读取"运动时长"字段，转换为分钟数

【如果是类型B - 每日步数总结】：
1. 标题通常为"步数"或类似字样
2. exercise_type 固定返回 "每日步数"
3. duration_min 固定返回 0（因为是全天累计，没有单次时长）
4. 重点识别：总步数（大字显示，如15,796步）
5. 读取消耗卡路里（如673千卡）
6. 注意数字可能有千位分隔符（如15,796需要转为15796）

第二步 - 读取时间数据：
1. 【单次运动】查找"时间""时长""用时"等字样，格式：MM:SS或HH:MM:SS，转换为总分钟数
2. 【步数总结】固定为0（全天累计无单次时长）
3. 如果看到时间范围如"00:00-23:59"，说明这是步数总结，不是单次时长

第三步 - 读取卡路里：
1. 查找"卡路里""消耗""千卡""kcal"等字样
2. 数值可能带千位分隔符（如1,234要转为1234）

第四步 - 读取步数：
1. 【单次运动】查找该次运动的步数
2. 【步数总结】查找大字显示的总步数（通常是最显眼的数字）
3. 注意去掉千位分隔符

第五步 - 读取可选数据：
1. 平均心率：查找"平均心率""avg""bpm"等
2. 最大心率：查找"最大心率""max"等
3. 日期：查找日期信息（如"11月10日""2025年10月28日"等）
...
{
  "exercise_type": "运动类型名称（单次运动填具体名称，步数总结固定填'每日步数'）",
  "duration_min": 运动时长分钟数(整数，步数总结固定为0),
  "calories": 消耗卡路里(整数),
  "steps": 步数(整数或null),
...
def validate_and_fix_data(data: Dict) -> Dict:
    ...
    # 判断是否为每日步数总结
    is_daily_steps = exercise_type == "每日步数"
    
    duration_min = data.get("duration_min")
    if is_daily_steps:
        # 每日步数总结：时长固定为0（全天累计，无单次时长）
        if duration_min != 0:
            duration_min = 0
    else:
        # 单次运动记录：时长必须大于0
